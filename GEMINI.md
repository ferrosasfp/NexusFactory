...
- **Fix**: 
  1. Archivo debe ser `middleware.ts` en raíz, exportando `middleware()`
  2. Solo setear cookies en `response.cookies`, nunca en `request.cookies`
  3. Al hacer redirect, copiar cookies del response original al redirectResponse
- **Aplicar en**: Todos los proyectos Next.js 13+ con Supabase SSR

### 2026-01-23: Next.js 16 elimina `next lint` y lint ya no corre en `next build`
- **Error/Síntoma**: En Next.js 16, `next lint` fue removido y `next build` ya no ejecuta lint; el QA puede quedar “PASS parcial” si no se corre ESLint explícitamente.
- **Fix**: 
  1) Usar ESLint CLI: `"lint": "eslint . --max-warnings 0"` y `"lint:fix": "eslint . --fix"`
  2) Usar Flat Config (`eslint.config.mjs`) con `eslint-config-next/core-web-vitals` (+ `eslint-config-next/typescript` si aplica).
  3) (Recomendado) Blindar el build: `"build": "npm run lint && next build"`
- **Aplicar en**: Todos los proyectos Next.js 16+ (Windows y CI).
